...
BEGIN
    ...
    SELECT @startDate = MIN([DateTime]), @endDate = MAX([DateTime]), @code = MAX([Code])
    FROM   @stockSummaryTable

    DELETE     [sst]
    FROM @stockSummaryTable [sst]
    JOIN(SELECT [DateTime]
         FROM   [summary] WITH (NOLOCK)
         WHERE  [DateTime] >= @startDate AND [DateTime] <= @endDate AND [code] = @code) [s] ON [s].datetime = [sst].datetime

    INSERT INTO          [Summary]([DateTime], [Code], [OpenPrice], [HighestPrice], [LowestPrice], [ClosePrice], [Volume], [Amount])
    SELECT [DateTime], [Code], [OpenPrice], [HighestPrice], [LowestPrice], [ClosePrice], [Volume], [Amount]
    FROM   @stockSummaryTable
...
[dbo].[usp_SaveStockSummary] '<SummaryItems>
	<Item>
