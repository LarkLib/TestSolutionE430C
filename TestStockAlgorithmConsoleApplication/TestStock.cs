...
namespace TestStockAlgorithmConsoleApplication
{
    public class TestStock
    {
        ...
        private readonly string[] stockList = ConfigurationManager.AppSettings["StockList"]?.Split(new[] { ',' });
        ...
        public void ProcessTest()
        {
            ...
            foreach (var role in roleList)
            {
                foreach (var item in stockList)
                {
                    ...
                    TestMethod(config, priceTransactions);
                    priceTransactions = null;
                }
            }
        }
        private void TestMethod(PriceConfig config, List<PriceTransactionItem> priceTransactions)
        {
            ...
            var dateList = GetDateList(startDate, code);
            foreach (var date in dateList)
            {
                //if (date.CompareTo("2015-01-15") > 0) break;
                var dateTime = DateTimeOffset.Parse(date);
                var priceItemList = GetPriceItemList(date, code);

                if (priceItemList.Any(p => p.PriceDateTime > dateTime))
                {
                    var basePrice = GetBasePrice(dateTime, code) ?? priceItemList[0].Price;
        ...
        public IList<string> GetDateList(DateTimeOffset startDate, string code)
        {
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["StockConnectionString"].ToString()))
            {
        ...
        public decimal? GetBasePrice(DateTimeOffset dateTime, string code)
        {
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["StockConnectionString"].ToString()))
            {
                SqlCommand cmd = conn.CreateCommand();              //创建SqlCommand对象
                ...
                cmd.CommandText = $@"
                      ...
                      FROM [Stock].[Summary] with(nolock)
        ...
        public IList<PriceItem> GetPriceItemList(string date, string code)
        {
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["StockConnectionString"].ToString()))
            {
                SqlCommand cmd = conn.CreateCommand();              //创建SqlCommand对象
                ...
                cmd.CommandText = $@"
                      ...
                      FROM 
                      (
                         SELECT TOP {LengthForDeltaIncreaseRole} [DateTime],[Price],[Id],[Volume],[Amount]
                         FROM [Stock].[Detail] with(nolock)
